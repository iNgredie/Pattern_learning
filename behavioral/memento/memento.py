...
class Originator:
    ...
    меняться. Он также объвляет метод сохранения состояния внутри снимка и
    метод свосстановления состояния из него.
    """

    _state = None
    """
    Для удобства состояние создателя хранится внтури одной переменной.
    """
    ...
    def do_something(self) -> None:
        ...
        Поэтому клиент должен выполнять резервное копирование состояния с
        помощью метода save перед запуском методов бизнес-логики.
        """

        print('Originator: I`m doing something important.')
        self._state = self._generate_random_string(30)
        print(f'Originarot: and my state has change to: {self._state}')
    ...
    def restore(self, memento: Memento) -> None:
        """
        Восстанавливает состояние Создателя из обхекта снимка.
        """
