...
class ConcreteComponentA(Component):
    ...
    def accept(self, visitor: 'Visitor') -> None:
        """
        Обратите внимание, мы вызываем visitConcteteComponentA, что
    ...
    def exclusive_method_of_concrete_component_a(self) -> str:
        ...
        базовом классе или интерфейсе. Поситель все же может использовать эти
        методы, посколько он знает о конкретном классе компонента.
        """
...
class Visitor(ABC):
    """
    Интерфейс Поситителя объявляет набор методов посещения, соотвестующих
...
которые могут работать со всеми классами конкрентых компонентво.

Максимальную выгоду от паттерна Посититель вы почувствуете, используя его со
сложной структурой объектов, такой как дерево Компоновщика. В этом случае было
бы полезно хранить некоторое промежуточное состояние алгоритма при выполнение
методов посетителя надо различными объектами структуры.
"""
