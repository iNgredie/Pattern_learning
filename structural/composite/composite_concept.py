...
class Component(ABC):
    ...
    def parent(self, parent: 'Component'):
        """
        При необходимости базовой Компонет может объявить интерфейс для
        установки и получения родителя компонета в древовидной структуре. Он
        также моежт предоставить некоторую реализацию по умолчанию для этих
        методов
        """
        self._parent = parent
    ...
    def is_composite(self) -> bool:
        """
        Вы можете предоставить метод, который позволит клиетскому коду понять,
        может ли компонет иметь вложенные объекты.
        """
        return False
...
class Leaf(Component):
    ...
    иметь вложенныех компонентов.

    Обычно обхекты Листьев выполняют фактическую работу, тогда как объекты
...
class Composite(Component):
    """
    Класс Контейнер содержит сложные компоненты, которые могут иметь влоеженные
    ...
    """
    Объект контейнера может как добавлять компоненты в свой списко вложенных
    ...
    def operation(self) -> str:
        """
        Контейнер выполняет свою основную логику особым образом. Он проходим
        рекурсивно через всех своих детей, собирая и суммируя их результаты.
        Поскольку потомки контейнера передают эти вызовы своим потомкам и так
        далее, в результате обходится все дерево объектов.
        """

        result = []
        for child in self._children:
            result.append(child.operation())
        return f'Ветка({"+".join(result)})'
